...
static void MountFilesystems() {
  ...
  if (homedir_from_env != NULL) {
    ...
    if (strcmp(homedir_from_env, "/") != 0) {
        CreateTarget(homedir_from_env + 1, true);
        if (mount("tmpfs", homedir_from_env + 1, "tmpfs",
                  MS_NOSUID | MS_NODEV | MS_NOATIME, NULL) < 0) {
          DIE("mount(tmpfs, %s, tmpfs, MS_NOSUID | MS_NODEV | MS_NOATIME, NULL)",
              homedir_from_env + 1);
        }
  ...
  if (homedir != NULL &&
    ...
    if (strcmp(homedir, "/") != 0) {
        CreateTarget(homedir + 1, true);
        if (mount("tmpfs", homedir + 1, "tmpfs",
                  MS_NOSUID | MS_NODEV | MS_NOATIME, NULL) < 0) {
          DIE("mount(tmpfs, %s, tmpfs, MS_NOSUID | MS_NODEV | MS_NOATIME, NULL)",
              homedir + 1);
        }
...
static void MakeFilesystemMostlyReadOnly() {
  ...
  while ((ent = getmntent(mounts)) != NULL) {
    ...
    // Skip the sandbox root mount, since that is considered privately writable
    if (strcmp(ent->mnt_dir, opt.sandbox_root_dir) == 0) {
      continue;
    }
...
int Pid1Main(void *sync_pipe_param) {
  ...
  SetupMountNamespace();
  SetupUserNamespace();
  // SetupUtsNamespace(); don't do this, breaks socket.gethostbyname
  MountFilesystems();
  MakeFilesystemMostlyReadOnly();
  // MountProc(); do this right after the sandbox root is mounted
  SetupNetworking();
  EnterSandbox();
  SetupSignalHandlers();
  SpawnChild();
