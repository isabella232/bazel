...
#
# We depend on python because bazel.py uses the system Python.

fpm --verbose --debug --prefix / -C $pkg_dir --deb-no-default-config-files -d python2.7 -d gcc -s dir -t deb -n "bazel" --provides bazel --conflicts bazel-beta --replaces bazel-beta -v 1.0.$timestamp usr etc

# Beta package
fpm --verbose --debug --prefix / -C $pkg_dir --deb-no-default-config-files -d python2.7 -d gcc -s dir -t deb -n "bazel-beta" --provides bazel --conflicts bazel --replaces bazel -v 1.0.$timestamp usr etc
